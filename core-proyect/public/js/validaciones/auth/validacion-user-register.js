...
const pattern = new RegExp(/^[A-Za-z\u00C0-\u017F\.\-]+$/i);
...
namee.addEventListener("input", function (event) {
  ...
  if (event.target.value.length < 1) {
    ...
    nameSpan.innerHTML = "Este campo debe contener únicamente caracteres alfabéticos";
    nameSpan.style.color = "red";
    namee.style.border = "1px solid red";  
  } else if (
...
lastname.addEventListener("input", function (event) {
  if (event.target.value.length < 1) {
    ...
    lastNameSpan.innerHTML = "Este campo debe contener únicamente caracteres alfabéticos";
    lastNameSpan.style.color = "red";
    lastname.style.border = "1px solid red";  
  } else if (
    event.target.value.length >= 3 &&
...
phone.addEventListener("input", function (event) {
  if (event.target.value.length < 1) {
    ...
    (event.target.value.length < 10 || event.target.value.length > 16)
  ) {
